...
while num != r:
    ...
    num = int(input("let's see if you are the next Ori Galler, guess the number the computer choose: "))
print(
    f"you have choosen cureectly the number that the computer choose is: {num} and you are an idiot who just waisted time from your life")
print(f"\n")
...
s = int(input(
    "if the number is higer then yours enter 1 if the number is lower then yours enter 2 and if the number is your number enter 10:"))
count = 1
while s != 10:
    if s == 1:
        if max > r:
...
# 10.6
def how_many_uppers_and_lowers(sting):  # Method
    countu = 0
    countl = 0
    for i in sting:
        if i.islower():
        ...
        if i.isupper():
            ...
            continue
    print(f"there is {countu} capital latter's and {countl} normal ones")
